...
{
	if (_displayMode != DisplayMode_AllInOneRow)
		return;
...
void MultiTabWidget::renameLabel(QWidget *widget, const QString &newLabel)
{
	...
	if (_displayMode == DisplayMode_AllInOneRow && widget2Info[widget].showSuperLabel)
...
void MultiTabWidget::focusWidget(QWidget *widget)
{
	...
	if (tabWidget)
	{
		if (_displayMode == DisplayMode_Hierarchical && tabWidgetMain)
