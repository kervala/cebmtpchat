...
void ChannelWidget::colorizeChatItems(const QColor &color)
{
   ...
   _textEditOutput->setPalette(palette);
    ...
    // Filter
    palette = _comboBoxFilter->palette();
    palette.setColor(QPalette::Base, color);
    _comboBoxFilter->setPalette(palette);
}
...
void ChannelWidget::sessionLogged()
{
	...
	QString compiler = QString("%1 on %2").arg(QSysInfo::prettyProductName()).arg(QSysInfo::currentCpuArchitecture());
